...
const _require$1 = node_module.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('chunks/lib-d0e65f8e.js', document.baseURI).href)));
const ensureElectronEntryFile = (root = process.cwd()) => {
    const pkg = path__default["default"].join(root, 'package.json');
    if (fs__default["default"].existsSync(pkg)) {
        ...
        if (!main) {
            ...
            const entryPath = path__default["default"].resolve(root, main);
...
function getElectronPath() {
    ...
    if (!electronExecPath) {
        const electronModulePath = path__default["default"].dirname(_require$1.resolve('electron'));
        const pathFile = path__default["default"].join(electronModulePath, 'path.txt');
        let executablePath;
        if (fs__default["default"].existsSync(pathFile)) {
            executablePath = fs__default["default"].readFileSync(pathFile, 'utf-8');
        }
        if (executablePath) {
            electronExecPath = path__default["default"].join(electronModulePath, 'dist', executablePath);
...
function findLibEntry(root, scope) {
    for (const name of ['index', scope]) {
        for (const ext of ['js', 'ts', 'mjs', 'cjs']) {
            const entryFile = path__default["default"].resolve(root, 'src', scope, `${name}.${ext}`);
            if (fs__default["default"].existsSync(entryFile)) {
                return entryFile;
            }
        }
    }
...
function findInput(root, scope = 'renderer') {
    const rendererDir = path__default["default"].resolve(root, 'src', scope, 'index.html');
    if (fs__default["default"].existsSync(rendererDir)) {
        return rendererDir;
    }
    return '';
}
    ...
    return [
        {
            ...
            config(config) {
                ...
                const defaultConfig = {
                    build: {
                        outDir: path__default["default"].resolve(root, 'out', 'main'),
                        target: nodeTarget,
                        assetsDir: 'chunks',
                        rollupOptions: {
        ...
        },
        {
            ...
            configResolved(config) {
                ...
                if (!lib) {
                    const rollupOptions = build.rollupOptions;
                    if (!(rollupOptions === null || rollupOptions === void 0 ? void 0 : rollupOptions.input)) {
                        throw new Error('build lib field required for the electron vite main config');
                    }
                    else {
                        const output = rollupOptions === null || rollupOptions === void 0 ? void 0 : rollupOptions.output;
                        if (output) {
    ...
    return [
        {
            ...
            config(config) {
                ...
                const defaultConfig = {
                    build: {
                        outDir: path__default["default"].resolve(root, 'out', 'preload'),
                        target: nodeTarget,
                        assetsDir: 'chunks',
                        rollupOptions: {
                            external: ['electron', ...node_module.builtinModules.flatMap(m => [m, `node:${m}`])],
    ...
    return [
        {
            ...
            config(config) {
                ...
                const emptyOutDir = () => {
                    ...
                    if (outDir) {
                        if (!path__default["default"].isAbsolute(outDir)) {
                            outDir = path__default["default"].resolve(root, outDir);
                        }
                        const resolvedRoot = vite.normalizePath(path__default["default"].resolve(root));
                ...
                const defaultConfig = {
                    build: {
                        outDir: path__default["default"].resolve(root, 'out', 'renderer'),
                        target: chromeTarget,
                ...
                config.build = buildConfig;
                config.envDir = config.envDir || path__default["default"].resolve(root);
        ...
        },
        {
            ...
            configResolved(config) {
                if (config.base !== './' && config.base !== '/') {
                    config.logger.warn(colors__default["default"].yellow('should not set base field for the electron vite renderer config'));
                }
                const build = config.build;
                if (!build.target) {
                ...
                const rollupOptions = build.rollupOptions;
                if (!rollupOptions.input) {
                    config.logger.warn(colors__default["default"].yellow(`index.html file is not found in ${colors__default["default"].dim('/src/renderer')} directory`));
        ...
        renderChunk(code, chunk) {
            if (code.match(nodeWorkerAssetUrlRE)) {
                ...
                while ((match = nodeWorkerAssetUrlRE.exec(code))) {
                    ...
                    let outputFilepath = path__default["default"].posix.relative(path__default["default"].dirname(chunk.fileName), filename);
...
function resetOutDir(config, outDir, subOutDir) {
    var _a;
    let userOutDir = (_a = config.build) === null || _a === void 0 ? void 0 : _a.outDir;
    if (outDir === userOutDir) {
        userOutDir = path__default["default"].resolve(config.root || process.cwd(), outDir, subOutDir);
        if (config.build) {
...
async function loadConfigFromFile(configEnv, configFile, configRoot = process.cwd(), logLevel, ignoreConfigWarning = false) {
    ...
    const resolvedPath = configFile
        ? path__default["default"].resolve(configFile)
    ...
    try {
        ...
        if (!ignoreConfigWarning && configRequired.length > 0) {
            vite.createLogger(logLevel).warn(colors__default["default"].yellow(`${configRequired.join(' and ')} config is missing`));
        }
        return {
        ...
        vite.createLogger(logLevel).error(colors__default["default"].red(`failed to load config from ${resolvedPath}`), { error: e });
        throw e;
    }
}
function findConfigFile(configRoot, extensions) {
    for (const ext of extensions) {
        const configFile = path__default["default"].resolve(configRoot, `${CONFIG_FILE_NAME}.${ext}`);
        ...
        plugins: [
            {
                ...
                setup(build) {
                    build.onResolve({ filter: /.*/ }, args => {
                        const id = args.path;
                        if (id[0] !== '.' && !path__default["default"].isAbsolute(id)) {
                            return {
                                external: true
                            };
                        }
            ...
            },
            {
                ...
                setup(build) {
                    build.onLoad({ filter: /\.[cm]?[jt]s$/ }, async (args) => {
                        ...
                        const injectValues = `const ${dirnameVarName} = ${JSON.stringify(path__default["default"].dirname(args.path))};` +
...
const _require = node_module.createRequire((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('chunks/lib-d0e65f8e.js', document.baseURI).href)));
async function loadConfigFormBundledFile(configRoot, configFile, bundledCode, isESM) {
    if (isESM) {
        const fileNameTmp = path__default["default"].resolve(configRoot, `${CONFIG_FILE_NAME}.${Date.now()}.mjs`);
        fs__default["default"].writeFileSync(fileNameTmp, bundledCode);
        ...
        try {
            ...
            try {
                fs__default["default"].unlinkSync(fileNameTmp);
            }
            catch { }
        }
    }
    else {
        const extension = path__default["default"].extname(configFile);
        const realFileName = fs__default["default"].realpathSync(configFile);
