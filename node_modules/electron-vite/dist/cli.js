...
cli
    ...
    const { createServer } = await Promise.resolve().then(function () { return require('./chunks/lib-e5edfe56.js'); });
    ...
    try {
        ...
        const error = e;
        vite.createLogger(options.logLevel).error(colors__default["default"].red(`error during start dev server and electron app:\n${error.stack}`), { error });
        process.exit(1);
    }
});
// build
cli.command('build [root]', 'build for production').action(async (root, options) => {
    const { build } = await Promise.resolve().then(function () { return require('./chunks/lib-c2c1cedb.js'); });
    const inlineConfig = createInlineConfig(root, options);
    try {
        ...
        const error = e;
        vite.createLogger(options.logLevel).error(colors__default["default"].red(`error during build:\n${error.stack}`), { error });
        process.exit(1);
    }
});
// preview
cli
    ...
    .action(async (root, options) => {
    const { preview } = await Promise.resolve().then(function () { return require('./chunks/lib-5506705a.js'); });
    const inlineConfig = createInlineConfig(root, options);
    try {
        ...
        const error = e;
        vite.createLogger(options.logLevel).error(colors__default["default"].red(`error during preview electron app:\n${error.stack}`), { error });
        process.exit(1);
    }
});
cli.help();
